...
import { validatePolygon } from "../validations/polygonValidation";
...
// POST (create) new polygon
router.post("/", validatePolygon, async (req, res) => {
  const { name, geometry } = req.body;

  const polygon = new Polygon({ name, geometry });
  await polygon.save();
  res.status(201).json(polygon);
});

// PUT (update) polygon
router.put("/:id", validatePolygon, async (req, res) => {
  const { name, geometry } = req.body;
